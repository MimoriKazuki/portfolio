...
- **仕様（Phase 2 確定・更新）**：環境変数 `ADMIN_EMAIL`（カンマ区切り）に登録された email を持つユーザーを管理者として判定する（ホワイトリスト方式）
- **実装ファイル**：`app/lib/auth/admin-guard.ts`（`isAdminEmail` / `requireAdmin` / `isAdminGuardErr`）
- **email 比較**：case-insensitive（OAuth プロバイダーが返す email の大文字小文字差で管理者ロックアウトを起こさないため）
- **多層防御**：
  - middleware（`/admin` 配下）：`auth.getUser()` で未認証チェック → `/auth/login?returnTo=...` へリダイレクト
  - API route / Page 個別：`requireAdmin()` を呼び出し ADMIN_EMAIL 照合（middleware を回避された場合の保険）
- **レスポンス**：
  - 未ログイン：`{ ok: false, status: 401, error: 'unauthorized' }`
  - 一般ユーザー（ADMIN_EMAIL 非該当）：`{ ok: false, status: 403, error: 'forbidden' }`
  - 管理者：`{ ok: true, user: { id, email } }`

#### Phase 1 → Phase 2 の変更経緯（業務的事情）

Phase 1 では「`auth.users` にレコードがある = 管理者」と設計したが、Phase 2 着手時の本番運用で **OAuth 経由で一般ユーザーも `auth.users` に登録される運用実態**が判明 → Phase 1 前提が崩れ、ロックアウト不能・全員管理者化のリスクが発生。

A 案（ADMIN_EMAIL ホワイトリスト方式）で置き換え、運用負担（環境変数管理）と引き換えに確実性を取った。本番では `ADMIN_EMAIL=sales@landbridge.co.jp,tamogami@landbridge.co.jp` 形式で運用。
