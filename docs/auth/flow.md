...
| `/`（トップ・LP） | 不要 |
| `/e-learning`（LP） | 不要 |
| `/e-learning/courses`（コース一覧） | 不要（閲覧可・購入導線で 401） |
| `/e-learning/courses/[slug]`（コース詳細） | 不要（is_free 動画は再生可・購入動画はログイン要求 UI） |
| `/e-learning/contents/[id]`（単体動画詳細） | 不要 |
...
- 退会 API は Phase 1 では公開しない（必要になればサービス層に追加）
- DB 側はソフトデリート（`e_learning_users.deleted_at`）の準備のみ済（schema.dbml）
- L1 確定：同一メールで再登録時は過去履歴を引き継ぐ。再ログイン時に `deleted_at` を NULL に戻す再活性ロジックを設ける（実装は Phase 2 で詳細化）
