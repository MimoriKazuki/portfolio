...
- 既存 shadcn `button.tsx` を**無視して**独自 Button を新規作成する（cva パターン破棄・任意 className 直書き等）
  - 補足（2026-05-13 追記・team-lead）：既存 shadcn の cva パターン・variants 構造・トークン経由スタイリングを **踏襲した上での** Atomic Design 階層化（`app/components/ui/button.tsx` を出発点に `app/components/atoms/Button.tsx` を新設し、variant 命名整理や状態 prop 追加に留める）は **許容**。NG なのは「既存パターンを無視してゼロから別ロジックの Button を作る」こと。判断指針：cva 構造維持＋トークン経由＋既存 ui/button.tsx を破壊しない、なら OK。
