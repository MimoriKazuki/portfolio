...
- 視聴画面の動画切替は URL パスで管理（`/courses/[slug]/videos/[videoId]`）
...
| `/e-learning/mypage/purchases` | `from`, `to`, `type`（course / content / all） | `from=2026-01-01` |
| `/admin/e-learning/purchases` | `from`, `to`, `status`（completed / refunded）, `userId`, `targetType` | `status=refunded` |
...
3. 「購入する」→ Server Action `createCheckoutSession({ courseId })` → Stripe Checkout URL を返す → `window.location.href`
4. 決済完了 → Stripe Webhook（BE）→ `e_learning_purchases` 作成
5. Stripe からの `success_url` で `/e-learning/checkout/complete?session_id=...` に到達 → Server で session 確認・purchase レコード参照 → 該当コース／動画への導線提示
