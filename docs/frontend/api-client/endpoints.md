...
**重要**：本ドキュメントは「FE 側から見た呼び出し口」を整理するもので、エンドポイント本体の仕様確定は be-plan-mate が `docs/api/endpoints.md` で行う。本書では FE から呼び出すサーフェスのみ記述する。
...
**認証要件の前提**：
- 1-1（B001 LP）：**認証不要**（未ログイン可・公開データのみ）
- 1-2〜1-6（B002〜B014 会員系）：**認証必須**（middleware ガード + Server Component 内で `auth.users` セッション再確認）
- 1-7（C\* 管理）：**認証必須**（既存 `/admin` middleware ガード継続・auth.users セッション存在＝管理者）
...
#### 1-2. 認証必須・会員一覧（B002 / B003 / B006）

会員系一覧画面で利用する読み取り。コース一覧と単体動画一覧は **auth 要件が非対称** である点に注意。

- **`GET /api/courses`（コース一覧）は auth 必須**：未ログイン時は 401・`middleware.ts` が `/auth/login?returnTo=<元 URL>` にリダイレクト。B003 コース一覧（`/e-learning/courses`）は会員限定
- **`GET /api/contents`（単体動画一覧）は public 認証**：未ログインでも閲覧可（LP B001 でも同 API を利用する想定）
- **非対称設計の理由**：コースは「会員登録のうえで体系学習」を促す導線、単体動画は LP からの導入導線として公開する Phase 1 方針
- フィルタ（カテゴリ、無料／有料、並び替え、ページ）は両 API 共通
- ユーザーのブックマーク（`e_learning_bookmarks`）※認証必須
- ユーザーの進捗（`e_learning_progress`：完了済 ID セット）※認証必須
- ユーザーの `has_full_access` ※認証必須

> **まとめ**：`/api/courses` = auth 必須、`/api/contents` = public 認証。middleware ガード対象も同じ非対称で揃える（`routing/routes.md` 参照）。
...
#### 1-5. 単体動画詳細／視聴（B007・認証必須・視聴は権限要）
...
#### 1-6. マイページ（B011 / B012 / B013 / B014・認証必須）
...
FE が呼び出す Server Action の名前を仮置き。実装詳細・引数・戻り値は be-plan-mate の `docs/api/endpoints.md` で確定する。

**認証要件の前提**：
- すべての Server Action は **認証必須**（未ログインで呼び出された場合は Server Action 内で `requireE_LearningUser()` 同等のガードで弾く）
- 会員操作（2-1）は `auth.users` セッション必須・`e_learning_users` レコード必須
- 管理操作（2-2）は `auth.users` セッション必須（Phase 1 は管理者ロール分離なし＝確定事項 §9）

#### 2-1. 会員操作（認証必須・一般ユーザー）
...
| `GET /api/me/access?session_id=...` | B009 購入完了画面の Webhook 反映待ちポーリング | 認証必須／FE はクライアントから 2 秒間隔で最大 10 回呼ぶ／BE 側 access-service が担当（実装は `docs/backend/logic/services/access-service.md`）／レスポンスで該当 `e_learning_purchases` の存在＝視聴権限反映を返す |
...
- `client/checkout.ts`：Stripe URL 取得 + **B009 視聴権限反映ポーリング**（`GET /api/me/access?session_id=...` を 2 秒間隔・最大 10 回のクライアント fetch ループ。Server Action ではなく Route Handler 直接 fetch でクライアントから呼ぶ。タイムアウト判定もこのヘルパ内で完結）
...
実装方針：それぞれ Server Action を呼ぶだけのシン薄ラッパ。エラーは throw、UI 側で toast 表示。`client/checkout.ts` のポーリング部分のみ Route Handler 直接 fetch。
...
- BE API 設計（be-plan-mate 側）：`docs/api/endpoints.md`
