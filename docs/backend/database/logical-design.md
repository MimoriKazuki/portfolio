...
- **業務的に大事なレコード（コース・章・コース内動画・単体動画・カテゴリ・資料）** → 論理削除（`deleted_at TIMESTAMPTZ`）
- **個人データ（ブックマーク・進捗）** → 物理削除（軽量データ・履歴保持要件なし）
- **購入レコード** → **物理削除不可**（税務観点・領収書再発行・永続保持）
- **ユーザー** → 退会時データ保持ポリシーが未確定のため、Gate 4 までに確定（暫定：論理削除＋個人情報マスキング想定）
...
| 属性 | 意味 | 必須 | 一意性 | 不変 | 採番 | 履歴 | 削除 | PK候補 |
|------|------|------|--------|------|------|------|------|--------|
| id | 内部識別子 | ✓ | 全社 | ✓ | 自動採番（UUID） | 不要 | 不可 | ✓ |
| auth_user_id | Supabase Auth ユーザーID | ✓ | 全社 | ✓ | auth.users 由来 | 不要 | 不可 | - |
| email | メールアドレス | ✓ | 全社（実運用上） | - | 手入力／OAuth 由来 | 不要 | 論理（マスキング） | - |
| display_name | 表示名 | - | - | - | 手入力／OAuth 由来 | 不要 | 論理 | - |
| avatar_url | アバター画像URL | - | - | - | 手入力／OAuth 由来 | 不要 | 論理 | - |
| is_active | アクティブ状態フラグ | ✓ | - | - | デフォルト true | 不要 | 論理 | - |
| has_full_access | 全コンテンツ視聴可フラグ（旧 has_paid_access の後継） | ✓ | - | - | デフォルト false | 不要 | - | - |
| last_accessed_at | 最終アクセス日時 | - | - | - | アプリで更新 | 不要 | - | - |
| created_at | 作成日時 | ✓ | - | ✓ | 自動 | 不要 | - | - |
| updated_at | 更新日時 | ✓ | - | - | 自動 | 不要 | - | - |
| deleted_at | 削除日時（論理削除） | - | - | - | 退会時に設定 | 不要 | - | - |
...
|------|------|------|--------|------|------|------|------|--------|
| id | 内部識別子 | ✓ | 全社 | ✓ | 自動採番（UUID） | 不要 | 不可 | ✓ |
| name | カテゴリ名 | ✓ | - | - | 手入力 | 不要 | 論理 | - |
| slug | URL 用識別子 | ✓ | 全社 | - | 手入力 | 不要 | 論理 | - |
| description | 説明 | - | - | - | 手入力 | 不要 | 論理 | - |
| display_order | 表示順序 | ✓ | - | - | 手入力（整数） | 不要 | - | - |
| is_active | アクティブ状態フラグ | ✓ | - | - | デフォルト true | 不要 | - | - |
| created_at | 作成日時 | ✓ | - | ✓ | 自動 | 不要 | - | - |
| updated_at | 更新日時 | ✓ | - | - | 自動 | 不要 | - | - |
| deleted_at | 削除日時（論理削除） | - | - | - | 手動 | 不要 | - | - |
...
|------|------|------|--------|------|------|------|------|--------|
| id | 内部識別子 | ✓ | 全社 | ✓ | 自動採番（UUID） | 不要 | 不可 | ✓ |
| title | タイトル | ✓ | - | - | 手入力 | 不要 | 論理 | - |
| description | 説明 | - | - | - | 手入力 | 不要 | 論理 | - |
| thumbnail_url | サムネイル画像URL | - | - | - | 手入力／アップロード | 不要 | 論理 | - |
| video_url | 動画URL | ✓ | - | - | 手入力／アップロード | 不要 | 論理 | - |
| duration | 動画長（表示用文字列、例「10:30」） | - | - | - | 手入力 | 不要 | 論理 | - |
| category_id | カテゴリFK | - | - | - | 選択 | 不要 | - | - |
| is_free | 無料公開フラグ | ✓ | - | - | デフォルト false | 不要 | - | - |
| price | 価格（円・NULL は無料の意味で運用） | - | - | - | 手入力 | 不要 | 論理 | - |
| stripe_price_id | Stripe Price ID（One-time Price） | - | 全社 | - | Stripe 連携で取得 | 不要 | - | - |
| display_order | 表示順序 | ✓ | - | - | 手入力 | 不要 | - | - |
| is_published | 公開状態フラグ | ✓ | - | - | デフォルト true | 不要 | - | - |
| is_featured | 注目フラグ（ピックアップ表示） | ✓ | - | - | デフォルト false | 不要 | - | - |
| view_count | 累計視聴数（参考値） | ✓ | - | - | アプリで加算 | 不要 | - | - |
| created_at | 作成日時 | ✓ | - | ✓ | 自動 | 不要 | - | - |
| updated_at | 更新日時 | ✓ | - | - | 自動 | 不要 | - | - |
| deleted_at | 削除日時（論理削除） | - | - | - | 手動 | 不要 | - | - |
...
|------|------|------|--------|------|------|------|------|--------|
| id | 内部識別子 | ✓ | 全社 | ✓ | 自動採番（UUID） | 不要 | 不可 | ✓ |
| title | コースタイトル | ✓ | - | - | 手入力 | 不要 | 論理 | - |
| slug | URL 用識別子 | ✓ | 全社 | - | 手入力 | 不要 | 論理 | - |
| description | 説明（コース概要） | - | - | - | 手入力 | 不要 | 論理 | - |
| thumbnail_url | サムネイル画像URL | - | - | - | 手入力／アップロード | 不要 | 論理 | - |
| category_id | カテゴリFK（M2：必須） | ✓ | - | - | 選択 | 不要 | - | - |
| is_free | 無料公開フラグ | ✓ | - | - | デフォルト false | 不要 | - | - |
| price | 価格（円・NULL は無料の意味で運用） | - | - | - | 手入力 | 不要 | 論理 | - |
| stripe_price_id | Stripe Price ID（One-time Price） | - | 全社 | - | Stripe 連携で取得 | 不要 | - | - |
| display_order | 表示順序 | ✓ | - | - | 手入力 | 不要 | - | - |
| is_published | 公開状態フラグ | ✓ | - | - | デフォルト false（新規作成時は非公開デフォルト） | 不要 | - | - |
| is_featured | 注目フラグ | ✓ | - | - | デフォルト false | 不要 | - | - |
| created_at | 作成日時 | ✓ | - | ✓ | 自動 | 不要 | - | - |
| updated_at | 更新日時 | ✓ | - | - | 自動 | 不要 | - | - |
| deleted_at | 削除日時（論理削除） | - | - | - | 手動 | 不要 | - | - |
...
| title | 章タイトル | ✓ | - | - | 手入力 | 不要 | 論理 | - |
| description | 章説明 | - | - | - | 手入力 | 不要 | 論理 | - |
| display_order | コース内での順序 | ✓ | コース内一意 | - | 手入力（整数） | 不要 | - | - |
| created_at | 作成日時 | ✓ | - | ✓ | 自動 | 不要 | - | - |
| updated_at | 更新日時 | ✓ | - | - | 自動 | 不要 | - | - |
...
| title | 動画タイトル | ✓ | - | - | 手入力 | 不要 | 論理 | - |
| description | 動画説明 | - | - | - | 手入力 | 不要 | 論理 | - |
| thumbnail_url | サムネイル画像URL | - | - | - | 手入力／アップロード | 不要 | 論理 | - |
| video_url | 動画URL | ✓ | - | - | 手入力／アップロード | 不要 | 論理 | - |
| duration | 動画長（表示用文字列） | - | - | - | 手入力 | 不要 | 論理 | - |
| is_free | コース未購入者にも視聴可フラグ | ✓ | - | - | デフォルト false | 不要 | - | - |
| display_order | 章内での順序 | ✓ | 章内一意 | - | 手入力（整数） | 不要 | - | - |
| created_at | 作成日時 | ✓ | - | ✓ | 自動 | 不要 | - | - |
| updated_at | 更新日時 | ✓ | - | - | 自動 | 不要 | - | - |
...
| title | 資料タイトル | ✓ | - | - | 手入力 | 不要 | - | - |
| file_url | ファイルURL | ✓ | - | - | アップロード | 不要 | - | - |
| file_size | ファイルサイズ（バイト） | - | - | - | アップロード時取得 | 不要 | - | - |
| display_order | 表示順序（資料間の並び） | ✓ | (content_id or course_id) 内 | - | 手入力 | 不要 | - | - |
| created_at | 作成日時 | ✓ | - | ✓ | 自動 | 不要 | - | - |
| updated_at | 更新日時 | ✓ | - | - | 自動 | 不要 | - | - |
...
| stripe_session_id | Stripe Checkout Session ID | ✓ | 全社 | ✓ | Stripe 由来 | 不要 | - | - |
| stripe_payment_intent_id | Stripe Payment Intent ID（返金照合用） | - | 全社 | ✓ | Stripe Webhook 由来 | 不要 | - | - |
| amount | 支払金額（円・税込） | ✓ | - | ✓ | Stripe 由来 | 不要 | - | - |
| status | 購入ステータス（completed / refunded / failed 等） | ✓ | - | - | Webhook で更新 | 不要 | - | - |
| refunded_at | 返金日時（charge.refunded 受信時） | - | - | - | Webhook で更新 | 不要 | - | - |
...
| completed_at | 視聴完了日時（完了の事実を保持） | ✓ | - | - | アプリで設定 | 不要 | - | - |
| created_at | 作成日時 | ✓ | - | ✓ | 自動 | 不要 | - | - |
| updated_at | 更新日時 | ✓ | - | - | 自動 | 不要 | - | - |
...
- id（UUID PK）
- company_name（企業名）
- contact_person / contact_email / contact_phone（連絡先）
- contract_status（active / expired / pending・CHECK制約）
- contract_start_date / contract_end_date（契約期間）
- notes（メモ）
