...
### SC-REG-006: 未ログインユーザーが新マイページ B011 に直アクセス → A001 へ returnTo 付きリダイレクト

- 対応変更: B011 page.tsx に未ログイン時 `redirect('/auth/login?returnTo=...')` 実装（コミット 97df16e）
- 変更履歴: B012（`/lp/mypage/bookmarks`）/ B013（`/lp/mypage/progress`）は Kosuke FB（2026-05-15）により削除済み → それぞれ SC-REG-021（マイラーニング）に移管
- 検証手順:
...
- 期待結果: 未認証アクセスで正しい returnTo 付きログインページへリダイレクトされること
- ステータス: 📋 未着手
...
### SC-REG-017: C004 状態フィルタ追加後も既存カテゴリ並び替え・追加・削除が正常動作する（非破壊確認）

- 対応変更: C004 で CategoriesAdminClient に statusFilter state と filteredCategories フィルタロジックを追加（コミット 6a71060）。既存 `handleAdd` / `handleUpdate` / `handleDelete` / `moveCategory` は変更なし
- 変更内容: フィルタは表示制御のみ。DB への INSERT/UPDATE/DELETE 処理・router.refresh() は完全温存。moveCategory の disabled 判定に `statusFilter !== ''` が追加されたが、statusFilter='' 時の既存動作は変化なし
- 検証手順:
  1. 状態フィルタ「すべて（削除済を除く）」（statusFilter=''）で ArrowUp / ArrowDown ボタンが有効なことを確認
  2. カテゴリ行を ArrowUp で移動 → DB の display_order が更新されることを確認（ページリロードで確認）
  3. 「新規追加」で `[TEST_CATEGORY]` カテゴリを追加 → 一覧に表示されることを確認
  4. `[TEST_CATEGORY]` を削除 → 一覧から消えることを確認
- 期待結果: C004 追加前と同じ挙動で既存カテゴリ操作が動作すること
- 補足: **`[TEST_CATEGORY]` のみ追加・削除すること。既存 6 カテゴリ（人作成データ）は並び替えのみ許容（元の順序に戻すこと）**
- ステータス: 📋 未着手

---

### SC-REG-018: 非管理者が `/admin/e-learning/legacy-purchases` に直アクセス → `/admin/e-learning` へリダイレクト

- 対応変更: C011 page.tsx に `requireAdmin()` 多層防御実装（コミット aeb1da1）
- 変更内容: 管理者以外のセッション（`guard.ok === false && status === 403`）は `/admin/e-learning` へリダイレクト
- 検証手順:
  1. 一般ユーザー（管理者でない）でログイン
  2. `/admin/e-learning/legacy-purchases` へ直アクセス → `/admin/e-learning` にリダイレクトされることを確認
- 期待結果: 非管理者がレガシー購入レコードの内容を表示できないこと
- ステータス: 📋 未着手

---

### SC-REG-019: B002 コース一覧の CourseCard リンクが B004 コース詳細（`/e-learning/lp/courses/[slug]`）に正しく遷移する（404 にならない）

- 対応変更: b3d73b7 で CourseCard の `href` を `/e-learning/lp/courses/${slug}` に修正（修正前は `/lp/` パスが欠落し 404 が発生していた）
- 変更内容: B002 コース一覧ページ（`/e-learning/lp/courses`）の CourseCard コンポーネントの `href` プロパティが `/e-learning/lp/courses/${course.slug}` に統一
- 検証手順:
  1. ログイン済み状態で `/e-learning/lp/courses` にアクセス
  2. コース一覧に CourseCard が表示されることを確認（dev-seed コースが存在する場合）
  3. CourseCard の `<a>` 要素の href 属性が `/e-learning/lp/courses/[slug]` になっていることを確認
  4. CourseCard をクリック → `/e-learning/lp/courses/[slug]` に 200 で遷移することを確認（404 にならないこと）
- 期待結果: B002 から B004 への遷移が 200 で完了し、コース詳細ページのタイトルが表示されること
- 補足: **dev-seed のダミーコース（dummy-ai-intro 等）が投入済みであることが前提。コースゼロ状態では CourseCard が表示されないため確認不可**
- ステータス: 📋 未着手

---

### SC-REG-020: `/e-learning/lp/home` 追加後も `/lp/courses` / `/lp/videos` が引き続き表示される（非破壊確認）

- 対応変更: 8c8f610 / 4702f34 / 46cb25a / 1b89d29 で `/e-learning/lp/home` を新規追加。`/lp/courses`・`/lp/videos` の page.tsx / get-courses-list.ts / get-videos-list.ts は完全 touch なし
- 変更内容: 新路由の追加のみで既存路由への変更なし。MediaCard / MediaFilterSidebar は新規追加コンポーネントで既存の CourseCard / MediaListFilterBarClient とは独立
- 検証手順:
  1. `/e-learning/lp/courses` にアクセス → 200 OK で h1「コース一覧」が表示されることを確認
  2. `/e-learning/lp/videos` にアクセス → 200 OK で h1「単体動画一覧」が表示されることを確認
  3. 両画面で既存フィルタ（カテゴリチップ・無料/有料・キーワード）が正常動作することを確認
- 期待結果: 新統合一覧追加前と同じ挙動で `/lp/courses` / `/lp/videos` が動作すること
- ステータス: 📋 未着手

---

### SC-REG-021: 未ログインユーザーがマイラーニング（B012b）に直アクセス → A001 へ returnTo 付きリダイレクト

- 対応変更: `f5d09b3` / `c9c7b02` で B012b `/e-learning/lp/mypage/learning` を新規実装。page.tsx に `if (!user) redirect('/auth/login?returnTo=/e-learning/lp/mypage/learning')` 実装
- 変更内容: B012（ブックマーク独立メニュー）/ B013（視聴履歴メニュー）を廃止し、マイラーニングに統合した際に新規追加された認証ガード
- 検証手順:
  1. 未ログイン状態で `/e-learning/lp/mypage/learning` へアクセス
  2. `/auth/login?returnTo=%2Fe-learning%2Flp%2Fmypage%2Flearning` にリダイレクトされることを確認
  3. ログイン後に `/e-learning/lp/mypage/learning` にリダイレクトバックされることを確認（SC-SMK-012d と合わせて確認）
- 期待結果: 未認証アクセスで正しい returnTo 付きログインページへリダイレクトされること
- ステータス: 📋 未着手
...
| C004-NONDESTRUCTIVE-001（状態フィルタ追加後も既存カテゴリ並び替え・追加・削除が非破壊） | SC-REG-017 | 2026-05-14 |
| C011-AUTH-001（非管理者が /admin/e-learning/legacy-purchases に直アクセス → リダイレクト） | SC-REG-018 | 2026-05-14 |
| B002-B004-LINK-001（CourseCard href 修正 → B004 が 404 にならない）| SC-REG-019 | 2026-05-14 |
| B002-HOME-NONDESTRUCTIVE-001（/lp/home 追加後も /lp/courses /lp/videos が非破壊）| SC-REG-020 | 2026-05-14 |
| B012b-AUTH-001（マイラーニング未ログイン returnTo リダイレクト） | SC-REG-021 | 2026-05-15 |
