...
- 操作: CourseCard（`aria-label="[タイトル] の詳細を見る"` リンク）をクリック
- 確認内容:
  - `/e-learning/lp/courses/[slug]` に遷移し、h1 にコースタイトルが表示される（200 で描画）
  - 404 / 500 エラーにならないこと
  - `aria-label` に含まれるタイトルと h1 のタイトルが一致すること
  - thumbnail_url が NULL のカードでは PlayCircle プレースホルダが表示され、カードリンクは正常動作すること
- 補足: **コミット b3d73b7 で修正した `href` の回帰確認。dev-seed 未投入環境では CourseCard が表示されないためスキップ可**
- ステータス: 📋 未着手
...
#### SC-UAT-096: C004 カテゴリ管理 「削除済のみ」フィルタ選択 → 削除済バッジ行のみ表示
- 対象URL: `/admin/e-learning/categories`
- 前提: 管理者ログイン済み・deleted_at ありのカテゴリが DB に存在する（既存6カテゴリに削除済があれば流用、なければ別途確認）
- 操作: 状態フィルタ Select を「削除済のみ」に変更（`statusFilter = 'deleted'`）
- 確認内容:
  - deleted_at ありカテゴリ行のみ表示される
  - 各行に「削除済」バッジ（`bg-red-100 text-red-700`）が表示される
  - 有効・無効カテゴリ行が表示されない
  - 件数表示「N / M 件」が削除済件数に更新される
- 補足: **読み取り専用確認。カテゴリの deleted_at は既存データで確認できない場合スキップ可（削除済カテゴリがなければテスト不可）**
- ステータス: 📋 未着手

#### SC-UAT-097: C004 カテゴリ管理 「有効のみ」「無効のみ」フィルタ切替 → 表示行が変わる
- 対象URL: `/admin/e-learning/categories`
- 前提: 管理者ログイン済み・有効（is_active=true）・無効（is_active=false）のカテゴリが混在
- 操作:
  1. 状態フィルタ Select を「有効のみ」に変更 → is_active=true のカテゴリのみ表示されることを確認
  2. 状態フィルタ Select を「無効のみ」に変更 → is_active=false のカテゴリのみ表示されることを確認
  3. 「すべて（削除済を除く）」に戻す → 有効+無効の全非削除カテゴリが表示されることを確認
- 確認内容: 各フィルタで「N / M 件」件数が連動して更新される
- ステータス: 📋 未着手

#### SC-UAT-098: C004 カテゴリ管理 フィルタ中は並び替えボタンが disabled になる
- 対象URL: `/admin/e-learning/categories`
- 前提: 管理者ログイン済み・カテゴリが2件以上存在する
- 操作:
  1. 状態フィルタ Select を「有効のみ」に変更（statusFilter !== ''）
  2. カテゴリ行の ArrowUp / ArrowDown ボタンを確認
- 確認内容:
  - フィルタ適用中（statusFilter !== ''）は全行の ArrowUp / ArrowDown ボタンが `disabled`（opacity-30・cursor-not-allowed）になっている
  - ボタンをクリックしても `moveCategory` が呼ばれない
  - 「すべて（削除済を除く）」に戻すとボタンが有効になり上下移動できる
- ステータス: 📋 未着手

### 管理画面 レガシー購入レコード（C011）

#### SC-UAT-099: C011 レガシー購入レコード 行の「詳細」ボタンクリック → aside インライン展開
- 対象URL: `/admin/e-learning/legacy-purchases`
- 前提: 管理者ログイン済み・レガシー購入レコード（6件）が表示されている
- 操作: テーブル行の「詳細」ボタン（Ghost variant）をクリック
- 確認内容:
  - `role="region"` `aria-label="レガシー購入レコード詳細"` の aside が画面下部に展開される
  - aside に「ID」「user_id」「content_id」「Stripe Session ID」「note（業務メモ）」の各フィールドが表示される
  - content_id が NULL の行では「（NULL・全コンテンツ買い切り）」と表示される
  - stripe_session_id が NULL の行では「（NULL・不明）」と表示される
- ステータス: 📋 未着手

#### SC-UAT-100: C011 レガシー購入レコード 「閉じる」ボタン → aside が非表示になる
- 対象URL: `/admin/e-learning/legacy-purchases`
- 前提: SC-UAT-099 で aside が展開された状態
- 操作: 展開中の行の「閉じる」ボタン（Ghost variant・展開時にラベルが変わる）をクリック
- 確認内容:
  - aside が非表示になる（expandedId が null にリセット）
  - 別行の「詳細」ボタンをクリックすると、その行の aside が新たに展開される（同時に1件のみ展開）
- ステータス: 📋 未着手

#### SC-UAT-101: C011 レガシー購入レコード 編集・削除ボタンが存在しない（読み取り専用確認）
- 対象URL: `/admin/e-learning/legacy-purchases`
- 前提: 管理者ログイン済み
- 操作: テーブル行をすべて確認
- 確認内容:
  - テーブルのすべての行に編集ボタン・削除ボタンが存在しない
  - 存在するボタンは「詳細」（Ghost variant）のみ
- ステータス: 📋 未着手
