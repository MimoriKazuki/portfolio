...
#### SC-SMK-003b: B002 新統合一覧 未ログイン redirect 確認（`/e-learning/lp/home`）
- 対象URL: `/e-learning/lp/home`
- 認証: 未ログイン
- 確認内容:
  - `/auth/login?returnTo=%2Fe-learning%2Flp%2Fhome` にリダイレクトされる（middleware ガード対象）
  - 未認証で統合一覧のコンテンツが表示されない
- ステータス: 📋 未着手

#### SC-SMK-003c: B002 新統合一覧 ページ描画確認（`/e-learning/lp/home`）
- 対象URL: `/e-learning/lp/home`
- 前提: ログイン済み
- 確認内容:
  - h1「学びを探す」が表示される
  - `aside[aria-label="一覧フィルタ"]` 内に「種別」「カテゴリ」「価格」の各セクションが表示される
  - キーワード検索 Input（`aria-label="キーワード検索"`）が表示される
  - MediaGrid（コース＋単体動画の混在グリッド、または 0件の空状態）が表示される
- ステータス: 📋 未着手
...
- 補足（dev-seed コース投入済み環境）: CourseCard が表示される場合、各カードに `aria-label="[タイトル] の詳細を見る"` リンクと「コース」Badge が存在することを確認。サムネなしコースではプレースホルダアイコン（PlayCircle）が表示され崩れないこと（コミット b3d73b7 で追加）
- ステータス: 📋 未着手
...
- 補足（コミット 436d1ea で hero サムネ追加）: thumbnail_url が NULL のコースでは `relative aspect-video` コンテナ内に PlayCircle プレースホルダアイコンが表示され、レイアウト崩れがないことを確認。thumbnail_url ありのコースでは `<img>` が表示されることを確認（どちらの場合も 200 OK でページが描画されること）
- ステータス: 📋 未着手
