...
## 進捗ステータス（最終更新：2026-05-13 Sub 2d 完全クローズ・Sub 2e 着手）
...
| 2 | 認証基盤の整理 | 4 / 6 | 🔧 対応中（P2-AUTH-01・02・03・04 完了 / 05 着手中・06 未着手） |
...
| P2-AUTH-05 | アプリコード切替（access-service 新規 + has_paid_access → has_full_access 置換） | 🔧 対応中 | Sub 2d（access-service 新規 + e-learning 配下置換）+ Sub 2e（残り 8ファイル置換） |
...
| 2e | 他7ファイルの has_paid_access → has_full_access 置換 | 🔧 対応中 |
