...
export function CPPNShaderBackground() {
  const camera = useMemo(() => ({ position: [0, 0, 1] as [number, number, number], fov: 75, near: 0.1, far: 1000 }), []);
  
  return (
    <div className="absolute inset-0 -z-10 w-full h-full opacity-15" aria-hidden>
      <Canvas
        camera={camera}
      ...
      {/* White overlay to maintain white-based design */}
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-white/80 via-white/50 to-white/30" />
    </div>
  );
}
