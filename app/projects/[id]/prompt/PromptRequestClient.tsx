...
interface PromptRequestClientProps {
  projectId: string
  initialProject: Project | null
}

export default function PromptRequestClient({ projectId, initialProject }: PromptRequestClientProps) {
  const router = useRouter()
  const [projectData] = useState<Project | null>(initialProject)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/prompt-request', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (result.csvContent) {
          ...
          link.href = url
          link.download = `${projectData?.title || 'project'}_prompts.csv`
            ...
            <div className="bg-gray-100 rounded-lg p-6 mb-6">
              {projectData.thumbnail && (
                <div className="relative aspect-video mb-4 rounded-lg overflow-hidden">
                  ...
                  />
                  <div className={`absolute top-2 right-2 ${categoryColors[projectData.category]} text-xs px-2 py-0.5 rounded`}>
                    {categoryLabels[projectData.category]}
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="プロンプトに関するご要望やご質問がございましたらご記入ください"
                    />
                  </div>
                </div>
              </div>
